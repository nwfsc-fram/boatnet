<template>
  <div class="q-pa-md">
    ...
    <div
      style="border-left: 2px dashed gray; height: 150px; position: absolute; left: 1115px; z-index: -1000; padding-left: 5px; color: gray; font-weight: bold; ; margin-top: 0 "
    >100% of target</div>
    <div v-for="(row, i) in myData" :key="i">
      <div
        style="height: 22px; padding-left: 10px; font-weight: bold; color: white;display: inline-block; border-bottom: 1px dotted black"
        :style="{width: row.value * 4 + 'px', backgroundColor: row.color}"
        @click="goToTarget(i)"
      >{{ row.label }}</div>
      <span
        style="position: relative; left: -100px; color: white; font-weight: bold"
      >{{ (row.value / 200) * 100 }}% covered</span>
      <span
        style="font-weight: bold; position: relative; left: -80px"
      >{{ lessThanZero(row.value - 200) ? row.value - 200 : "+" + (row.value - 200).toString() }}</span>
    </div>
...
export default class OTSDashboard extends Vue {
  private myData: any[] = [
    ...
    {
      label: 'AT (Astoria, OR / Columbia River Ports)',
      value: 212,
      color: '#41b900'
    },
  ...
  private mounted() {
    ...
    const text = svg
      ...
      .enter()
      .append('text')
      .text( (d) => (d.value / 200) * 100)
      .attr('x', (d, i) => d.value * 4 - 30)
      .attr('y', (d, i) => barWidth * i + 18)
      .attr('fill', '#FFFFFF')
