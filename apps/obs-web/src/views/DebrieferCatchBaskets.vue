...
export default class DebrieferOperations extends Vue {
 ...
 private catchBasketsColumns = [
  ...
  private addAll() {
    this.visibleCatchBasketsColumns = this.catchBasketsColumns.map(
      (catchBasketsColumns) => catchBasketsColumns.name
    );
  }

  private removeAll() {
    this.visibleCatchBasketsColumns = [];
  }
}
</script>