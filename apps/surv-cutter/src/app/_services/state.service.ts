...
export class StateService {
  ...
  constructor(private router: Router) {
    ...
    router.events.subscribe((event: Event) => {
      if (event instanceof NavigationStart) {
        ...
        const urlIndex = router.config.findIndex(
          item => item['path'] === tempUrl
        );
        console.log(
          'url = ' +
            urlIndex +
            ', item is: ' +
            JSON.stringify(router.config[urlIndex])
        );
      ...
      if (event instanceof NavigationEnd) {
        ...
        console.log(
          'currentScreen: ' + this.formatScreen(event.url.replace('/', ''))
        );
  ...
  setAppConfig(category?: string, program?: string, gearType?: string) {
    ...
    this.appConfigObject.configureApp(
      this.category.getValue(),
      this.program.getValue()
    );
  ...
  formatScreen(screen: string) {
    ...
    return tmp.replace(/^./, function(str) {
