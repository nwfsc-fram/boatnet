...
export default createComponent({
    ...
    setup(props, context) {
        ...
        const getNextIndex = () => {
            if (index.value === options.length -1) { index.value = 0; }
            else { index.value += 1; }
        ...
        const getColor = computed(() => {
            if (index.value % 2) { return 'white'; }
            else { return 'primary'; }
        });

        const getTextColor = computed(() => {
            if (index.value % 2) { return 'primary'; }
            else { return 'white'; }
        });
        ...
        return {
            value, index, getNextIndex, getColor, getTextColor
        };
    }
});
</script>
